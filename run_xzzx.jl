...
let
    ...
    if mode == 0
        AsitesArr = genXZZXSqrIdx(1,str_length,1,1,dx,dy)
        CsitesArr = genXZZXSqrIdx(1,str_length,yi,yi,dx,dy)
    elseif mode == 1
        AsitesArr = genXZZXDiagIdx(1,yi,str_length,dx,dy,true)
        CsitesArr = genXZZXDiagIdx(1+offset,yi+offset,str_length+offset,dx,dy,false)
    end
